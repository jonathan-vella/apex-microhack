...
The arrows show workshop order, not automatic agent execution. At each APEX handoff, the team selects the next main agent, resolves required `10-Challenger` findings, and approves the transition. Challenge 3 compresses Governance, planning, code generation, validation, deployment authorization, and deployment into one workshop block.
