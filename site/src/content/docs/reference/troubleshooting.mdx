...
description:
  Common issues and solutions for environment setup, Azure deployments,
...
export const quickDiagnosisHeaders = ["Symptom", "Go to", "Likely category"];

export const quickDiagnosisRows = [
  [
    "Can't log in to Azure, subscription not found",
    '<a href="#azure-cli">Azure CLI</a>',
    "Auth / subscription issue",
  ],
  [
    "<code>RequestDisallowedByPolicy</code> error",
    '<a href="#policy-errors-and-governance">Policy Errors</a>',
    "Azure Policy denial",
  ],
  [
    "<code>QuotaExceeded</code>, region capacity errors",
    '<a href="#quota-errors">Quota Errors</a>',
    "Insufficient Azure quota",
  ],
  [
    "Container won't build, extensions missing",
    '<a href="#dev-container">Dev Container</a>',
    "Environment setup issue",
  ],
  [
    "<code>bicep build</code> fails, module errors",
    '<a href="#bicep">Bicep</a>',
    "Template/syntax issue",
  ],
  [
    "Deployment fails (naming, auth, timeout)",
    '<a href="#deployment-errors">Deployment Errors</a>',
    "Azure deployment issue",
  ],
  [
    "Agent not appearing, MCP errors",
    '<a href="#copilot-agents">Copilot Agents</a>',
    "Agent / tooling issue",
  ],
  [
    "Secret leaked in output or commit",
    '<a href="#secrets-and-credential-leakage">Secrets</a>',
    "Credential leakage",
  ],
  [
    "Lint or link check fails",
    '<a href="#validation-and-linting">Validation</a>',
    "Linting / validation",
  ],
];

Use this page to diagnose setup, Azure deployment, validation, and Accelerator agent issues during the MicroHack.
...
<ExpandableTable
  ...
  headers={quickDiagnosisHeaders}
  rows={quickDiagnosisRows}
  columnWidths={["40%", "26%", "34%"]}
  minWidth="52rem"
  expandedMinWidth="60rem"
/>
...
1. Ensure Docker Desktop is running and has at least 4 GB of RAM allocated
...
4. Open the current Accelerator agent files and confirm their declared models are permitted by your account or organization

### Agent model is not available

**Symptoms**: An Accelerator agent cannot start because its declared model is unavailable.

**Fixes**:

1. Open the affected `.github/agents/*.agent.md` file and note its current `model` value.
2. Check your Copilot plan entitlements and organization model policy.
3. Ask the GitHub organization or enterprise owner whether that model is permitted.
4. Reload VS Code after policy changes.

Do not replace the model declaration only to make the error disappear. Compare your repository with the current Accelerator and ask the facilitator before changing an upstream-managed agent.

### Agent not appearing in Chat

**Symptoms**: A main agent such as `01-Orchestrator` does not appear in the agent picker.

**Fixes**:

1. Confirm that you opened the template-derived repository, not this documentation repository.
2. Confirm the repository is open in its Dev Container.
3. Verify the expected files exist under `.github/agents/`.
4. Reload VS Code with `Developer: Reload Window`.
5. Compare the repository with the current APEX Accelerator if files are missing.

Do not enable a legacy discovery setting as a substitute for loading the current repository configuration.
...
3. Use `01-Orchestrator` to identify the next main agent and proposed handoff; the human still selects and approves that handoff

### MCP servers not working

**Symptoms**: A GitHub or Azure MCP server declared by the Accelerator returns an error or is unavailable.

**Fixes**:

1. Inspect `.vscode/mcp.json` in the template-derived repository and identify the failing server.
2. Preserve the exact startup or authentication error.
3. Confirm account and organization policy permits the declared server.
4. For Azure-backed tools, confirm the required Azure sign-in and subscription context.
5. Reload VS Code with `Developer: Reload Window`.
6. Compare your MCP configuration with the current APEX Accelerator.

The server list changes over time. Do not diagnose against the retired Azure Pricing, Draw.io, MS Learn, or Terraform list unless those servers are present in your current repository.
...
APEX reference: [MCP server integration](https://apexops.pro/concepts/how-it-works/mcp-integration/)
...
3. Preserve the exact error and escalate to the facilitator or the selected
   `07b-Bicep Deploy` or `07t-Terraform Deploy` agent. Participants must not
   substitute direct Azure CLI deployment for the approved agent path.
